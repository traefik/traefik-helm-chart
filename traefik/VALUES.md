...
![Version: 37.1.0](https://img.shields.io/badge/Version-37.1.0-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: v3.5.1](https://img.shields.io/badge/AppVersion-v3.5.1-informational?style=flat-square)
...
| hub.apimanagement.admission.selfManagedCertificate | bool | `false` | When this chart is installed with a CD tool such as ArgoCD, this tool won't be able to perform a diff or detect drifts during templating stage without `--dry-run=server` option passed to helm (which is not the case with ArgoCD). Thus, this option will help you to manage the certificate with other tools like `cert-manager` and not be annoyed by the CD tool complaining about changes at every run. An alternative with ArgoCD is to use `ignoreDifferences` setting. |
