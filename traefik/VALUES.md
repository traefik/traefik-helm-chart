...
| core.strictTLSOptions | bool | `false` | Disables the unsafe fallback to the default TLS options for routers with conflicting TLS options, which mitigates [GHSA-g55h-rg46-x9c5](https://github.com/traefik/traefik/security/advisories/GHSA-g55h-rg46-x9c5). Routers that currently rely on this fallback stop being served. Requires traefik v3.7.11+. |
...
| providers.kubernetesCRD.defaultTLSResourcesNamespace | string | `""` | Restricts the namespace in which the cluster-wide TLSOption and TLSStore named `default` can be defined. When empty, they are taken into account whatever their namespace. Resources named `default` outside of it are then ignored, certificates included. Requires traefik v3.7.11+. |
...
| providers.kubernetesCRD.safeNaming | bool | `false` | Enables collision-safe naming for the routers, middlewares and services generated by the provider. Defaults to `false`, which keeps the current collision-prone naming and silences the startup warning traefik logs when the option is left unset. Setting it to `true` renames the generated routers and services, so update dashboards, alerts and log queries first. Set to `null` to restore the traefik default and get the startup warning back. Requires traefik v3.7.11+. |
